...
class FrameParser:
    ...
    def payload_parse(self, input_data: bytes) -> FrameParseResult:
        ...
        if self.message_package is None or len(self.message_package) < 90:
            return None
        ...
        if self.receive_mode == "signal":
            ...
            for i in range(0, len(self.message_package), 1):
                ...
                if cmd_id == info.cmd_id_1:
                    ...
                    info.occupation_status = self.message_package[i + 6 : i + 12]
        ...
        if self.receive_mode == "noise":
            ...
            for i in range(0, len(self.message_package), 1):
                ...
                if cmd_id == noise_key.cmd_id_6:
                    ...
                    noise_key.sdr_key = int.from_bytes(
                        self.message_package[i + 3 : i + 9], byteorder="big"
                    )  # 6 byte
                else:
                    return None
            return noise_key
